 ...
 * 
 */
package com.iboot.jdbcx.dialect;
...
public class SelectSqlUtils {
	...
	public static void getPageableSqlWithLimitOffset(Select select, Pageable pageable) {
		...
		if (sb instanceof PlainSelect) {
			...
			limit.setRowCount(new LongValue(pageable.getPageSize()));
			limit.setOffset(new LongValue(pageable.getOffset()));
	...
	/**
	 * build sql OFFSET :offsetNum ROWS FETCH NEXT :pageSize ROWS ONLY
	 * @param select
	 * @param offsetNum
	 * @param pageSize
	 */
	private static void addOffsetFetch(Select select,long offsetNum,long pageSize) {
		...
		if (sb instanceof PlainSelect) {
			...
			offset.setOffset(offsetNum);
			offset.setOffsetParam("ROWS");
			plainSelect.setOffset(offset);
			
			Fetch fetch = new Fetch();
			boolean isFetchParamFirst = offsetNum<=0?true:false;
			fetch.setFetchParamFirst(isFetchParamFirst);
			fetch.setRowCount(pageSize);
			fetch.setFetchParam("ROWS");
			plainSelect.setFetch(fetch);
			
		} else if (sb instanceof SetOperationList) {
			SetOperationList setOperationList = (SetOperationList) sb;
			Offset offset = new Offset();
			offset.setOffset(offsetNum);
			offset.setOffsetParam("ROWS");
